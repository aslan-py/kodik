...
Гипотеза: раздел появляется в сайдбаре в порядке импорта модулей `api/admin/__init__.py`, поэтому `pipeline_control` ставится первым в списке импортов. Библиотека это нигде явно не документирует (в `fastadmin/models/base.py` нет сортировки по приоритету) — если на практике порядок окажется другим (например, алфавитным), это не меняет ни спеку, ни поведение системы, только видимое место пункта меню — фиксируется как Open Question, проверяется на реализации.
