...
| API-слой / админка | **FastAPI + SQLAlchemy (async) + pydantic**; авто-админка — **FastAdmin** (НЕ DRF) |
...
DRF синхронный и использует свои сериализаторы вместо pydantic. FastAPI + FastAdmin закрывают и API, и админку,
...
```jsonc
{
  "meta": {
    ...
    "items_count": 1,                                  // сколько материалов передано в BP-2
    "empty_reason": null                                // код причины пустого items, если материалов нет
  },
...
2. **Отдельную форму можно не делать.** `action_item` регистрируется в той же **админке** (FastAdmin), что и справочники BP-7,
...
(`competitor`, `trigger`, `source`, `black_domain`, `stop_word`, `routing_rule`, …). Взяли **FastAdmin**
(авто-генерация CRUD из моделей), CRUD руками не пишем. Так закрывается «добавление без кода» из ТЗ.

**Реализовано.** Админка живёт на `/admin` в том же процессе, что и API (отдельный сервер/контейнер не нужен),
код — в `api/admin/`, инструкция по запуску и разбор разделов — в `api/ADMIN_README.md`. Вход по email+паролю
для ролей `admin`/`analyst`. Справочники правятся, слои данных конвейера и витрина — только на просмотр.
