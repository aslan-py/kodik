...
Используется для чанков ``StructuredChunker`` (``LLMClient``) — сигнатура
не завязана на ``Chunk``, поэтому пригодна для любого списка задач с
ограничением параллелизма, без дублирования этой логики в других местах.
"""
