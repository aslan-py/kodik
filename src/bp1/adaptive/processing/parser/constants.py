...
# Сколько раз подряд использование закэшированного RSS/Atom/sitemap-фида
# может провалиться, прежде чем запись обнаружения сбрасывается (design.md
# изменения add-rss-sitemap-collection, D5) — тот же принцип, что
# ADAPTER_FAIL_THRESHOLD: разовый сбой (временная недоступность сайта) не
# должен сразу выбрасывать рабочий фид из кэша.
FEED_FAIL_THRESHOLD = settings.bp1_feed_fail_threshold
