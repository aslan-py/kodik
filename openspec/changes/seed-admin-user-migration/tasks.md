...
- [ ] 3.1 Сменить пароль администратора через `/admin` (кнопка смены пароля у `UserAdmin`) на средах, где к репозиторию есть посторонний доступ — захардкоженный `Admin123` не должен оставаться боевым паролем
