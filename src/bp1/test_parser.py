#!/usr/bin/env python
"""
Ручной отладочный скрипт для проверки BP-1 с реальной БД и Redis.

НЕ автотест: несмотря на имя функции `test_with_real_db` (историческое),
pytest его не подхватывает — `testpaths = ["tests"]` в `pyproject.toml`
сюда не заглядывает. Запускать вручную, когда нужно руками прогнать
парсинг на реальных задачах сбора и посмотреть результат:
